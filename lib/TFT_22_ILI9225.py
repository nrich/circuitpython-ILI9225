...
# ILI9225 screen size
ILI9225_LCD_WIDTH: int = 176
ILI9225_LCD_HEIGHT: int = 220
...
def convert_pin(pin: Pin|digitalio.DigitalInOut) -> digitalio.DigitalInOut:
...
#        self.setBacklight(True)
    ...
    def writeRegister(self, reg, data):
        ...
        self.cs.value = False           # self.cs(0)
        self.dc.value = False           # self.dc(0)
        self.spi.write(reg.to_bytes(2,'big'))
        self.dc.value = True            # self.dc(1)
        self.spi.write(data.to_bytes(2,'big'))
        self.cs.value = True            # self.cs(1)
